...
class TestRDBDataTable(unittest.TestCase):
    ...
    def test_find_by_primary_key_success(self):
        csv_tbl_s = RDBDataTable("people", CONNECT_INFO, PRIMARY_KEY_SINGLE_FILED)
        result_s = csv_tbl_s.find_by_primary_key(PRIMARY_KEY_SINGLE_VALUE)
        csv_tbl_m = RDBDataTable("people", CONNECT_INFO, PRIMARY_KEY_MULTIPLE_FIELD)
        ...
        csv_tbl_n = RDBDataTable("people", CONNECT_INFO, PRIMARY_KEY_SINGLE_FILED)
        result = csv_tbl_n.find_by_primary_key(["a"])
        self.assertEqual(None, result)

    def test_find_by_primary_key_failure(self):
        csv_tbl = RDBDataTable("people", CONNECT_INFO, PRIMARY_KEY_SINGLE_FILED)
        ...
        csv_tbl = RDBDataTable("people", CONNECT_INFO, None)
        with self.assertRaises(Exception) as context:
            result = csv_tbl.find_by_primary_key([])
